...
public class Manager extends Employee {
			...
			for (Ticket t : x.getTicketList()) {
				if (t.getTrip().equals(trip)) {
					x.addMessage(this.userName, x.getUserName(), "Trip cancelation", "Sorrily the trip with number "
							+ t.getTrip().getNumber()
							+ " has been cancelled and trip price has been added back to your balance, please review your trips, sorry for inconvinience.");
	...
	public Driver assignTrip(Trip trip, Driver driver) {
		...
		driver.addMessage(this.userName, driver.userName, "Trip assign notification",
				"Trip number " + trip.getNumber() + " has been assigned to you");
		driver.addTrip(trip);
		return null;
	}
	...
	public void removeDriverFromTrip(Driver driver, Trip trip) {
		...
		driver.addMessage(this.userName, driver.userName, "Trip unassign notification",
				"Trip number " + trip.getNumber() + " has been removed from you and might be added to another driver.");
