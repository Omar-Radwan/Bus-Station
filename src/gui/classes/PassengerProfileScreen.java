									...
									public void handle(ActionEvent event) {
										if (ticketComboBox.getValue().equals("One way")) {
											priceValueLabel.setText(
													Double.toString(Ticket.priceInCaseOfOneWay(trip.getPrice())));
										} else {
											priceValueLabel.setText(
													Double.toString(Ticket.priceInCaseOfRound(trip.getPrice())));
		...
		cancelTripLink.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				if (!passenger.getTicketList().isEmpty()) {
					...
					showList(passenger.getTicketList(), 300, 200, "Hyperlink", "Red");
